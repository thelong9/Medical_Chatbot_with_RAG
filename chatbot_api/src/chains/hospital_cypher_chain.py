...
"""
graph: Đối tượng Neo4jGraph kết nối với Neo4j Instance
qa_prompt: prompt template để phản hồi các câu hỏi/truy vấn
cypher_promt: promt template để tạo các truy vấn Cypher
validate_cypher: Nếu là True, truy vấn Cypher sẽ được kiểm tra để sửa trước khi chạy(Note: truy vấn đó chưa chắc 
                đã valid vì nó chỉ sửa các lỗi cấu trúc đơn giản)
"""

hospital_cypher_chain = GraphCypherQAChain.from_llm(
    cypher_llm=ChatGoogleGenerativeAI(model=HOSPITAL_CYPHER_MODEL, temperature=0, google_api_key="<YOUR_API_KEY>"),  
    qa_llm=ChatGoogleGenerativeAI(model=HOSPITAL_QA_MODEL, temperature=0, google_api_key="<YOUR_API_KEY>"),
    graph=graph,
    verbose=True,
